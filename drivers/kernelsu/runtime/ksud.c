...
int ksu_handle_vfs_read(struct file **file_ptr, char __user **buf_ptr, size_t *count_ptr, loff_t **pos_ptr)
{
...
void ksu_handle_input_handle_event(unsigned int *type, unsigned int *code, int *value)
{
	if (type && code && value)
		vol_detector_event(NULL, *type, *code, *value);
}
