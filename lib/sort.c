 ...
 *
 * Ported from Linux 6.x by bandido-lineageos.
 */
 ...
 * be aligned if we do not have CONFIG_HAVE_EFFICIENT_UNALIGNED_ACCESS.
 *
 ...
 * instructions.
 */
#define SWAP_WORDS_64	((swap_r_func_t)(uintptr_t)0)
#define SWAP_WORDS_32	((swap_r_func_t)(uintptr_t)1)
#define SWAP_BYTES	((swap_r_func_t)(uintptr_t)2)
#define SWAP_WRAPPER	((swap_r_func_t)(uintptr_t)3)

#define _CMP_WRAPPER	((cmp_r_func_t)(uintptr_t)0)
		     ...
		     bool may_schedule)
{
	...
	for (;;) {
		...
		if (a)			/* Building heap: sift down a */
			...
			shift = do_cmp(base + size, base + 2 * size,
				       cmp_func, priv) <= 0;
			a = size << shift;
			n -= size;
 ...
 * is significantly faster for common cases.
 *
...
/**
 * sort_nonatomic - sort an array of elements, with cond_resched
 * @base: pointer to data to sort
 * @num: number of elements
 * @size: size of each element
 * @cmp_func: pointer to comparison function
 * @swap_func: pointer to swap function or NULL
 *
 * Same as sort, but preferred for larger arrays as it does a periodic
 * cond_resched().
 */
