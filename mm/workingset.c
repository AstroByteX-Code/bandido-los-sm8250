...
bool lru_gen_test_recent(void *shadow, bool file, struct lruvec **lruvec,
			 unsigned long *token)
{
	int memcg_id;
	bool workingset;
	...
	if (pgdat != lruvec_pgdat(*lruvec))
		return false;

	rcu_read_lock();

	if (!mem_cgroup_disabled()) {
		struct mem_cgroup *memcg = mem_cgroup_from_id(memcg_id);

		if (!memcg || memcg != lruvec_memcg(*lruvec)) {
			rcu_read_unlock();
			return false;
		}
	}
